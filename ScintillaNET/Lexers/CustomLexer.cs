...
namespace ScintillaNET.Lexers
{
	...
	public abstract class CustomLexer
	{
		...
		/// <summary>
		/// The internal reference to the scintilla control.
		/// </summary>
		...
		protected virtual void InitializeIndicators(IndicatorCollection indicators)
		{
			indicators[INDICATOR_ERROR].ForeColor = Color.Red;
			indicators[INDICATOR_ERROR].Under = false;
		...
		{
			if (length > 0)
				mScintilla.SetStyling(length, style);
		...
		protected void StartStyling()
		{
			if (!mLexerForcedRestyle)
				mScintilla.StartStyling(StartPosition);
		}
		...
		Start:
			...
			CurrentLine = mScintilla.LineFromPosition(startPosition);
			CurrentColumn = mScintilla.GetColumn(startPosition);
			CurrentPosition = 0;
			CurrentBasePosition = 0;
			CurrentState = STATE_UNKNOWN;
			mScintilla.IndicatorCurrent = INDICATOR_ERROR;
			mScintilla.IndicatorClearRange(startPosition, text.Length);

			UpdateKeywords();
			if (UseDynamicOptions)
				UpdateProperties();

			InitializeStateFromStyle(mScintilla.GetStyleAt(StartPosition + mScintilla.Lines.CharLineLength(CurrentLine - 1)));

			Style();
			if (mDelayedStyleText != null)
			{
				startPosition = mDelayedStyleStartPosition;
				text = mDelayedStyleText;
				mDelayedStyleText = null;
		...
		private int mDelayedStyleStartPosition = 0;
		private string mDelayedStyleText = null;
		protected void StyleNextLine()
		{
			var len = Scintilla.TextLength;
			mDelayedStyleStartPosition = StartPosition + mScintilla.Lines.CharLineLength(CurrentLine);
			var endPos = mDelayedStyleStartPosition + mScintilla.Lines.CharLineLength(CurrentLine + 1);
			if (endPos > len)
				endPos = len - 1;
			if (mDelayedStyleStartPosition > len)
				endPos = len - 1;
			if (mDelayedStyleStartPosition == endPos)
				return;
			this.mDelayedStyleText = mScintilla.Text.Substring(mDelayedStyleStartPosition, endPos - mDelayedStyleStartPosition);
		}
		...
		{
			mScintilla.IndicatorCurrent = INDICATOR_ERROR;
			mScintilla.IndicatorFillRange(StartPosition + position, length);
		}
		...
		{
			if (Scintilla.Keywords.Modified)
			{
				if (EnableCompositeKeywords)
				{
		...
		private uint mFoldDepth = NativeMethods.SC_FOLDLEVELBASE;
		protected uint FoldDepth { get { return mFoldDepth; } }
		protected void EnterFoldLevel()
		{
			mFoldDepth++;
			mCurrentLineFlags = NativeMethods.SC_FOLDLEVELHEADERFLAG;
		}
		...
		{
			if (mFoldDepth > NativeMethods.SC_FOLDLEVELBASE)
				LeaveFoldLevel();
		}
		private void UpdateCurrentFoldLevel()
		{
			if (mCurrentLineFlags != 0)
				mFoldDepth--;
			mScintilla.SetFoldLevel(CurrentLine, mFoldDepth | mCurrentLineFlags);
			if (mCurrentLineFlags != 0)
				mFoldDepth++;
		}
		#endregion
